...
def compute_hits_per_host(ls_filtered: DataFrame) -> DataFrame:
    return ls_filtered.groupBy("host").agg(F.sum("hits").alias("total_h_hits"),
                                            median_arr(F.collect_list("hits")).alias("median_h_hits"))

def compute_cond_p(ls_filtered: DataFrame, hits_per_h: DataFrame) -> DataFrame:
    hits_per_bh = ls_filtered.groupBy("app", "host").agg(F.sum("hits").alias("bh_hits"))
    return hits_per_h.join(hits_per_bh, "host").selectExpr("app", "host", "bh_hits/total_h_hits as cond_p", "bh_hits/median_h_hits as host_importance")
...
hits_per_h = compute_hits_per_host(ls_filtered)
cond_ps = compute_cond_p(ls_filtered, hits_per_h)
